...
private:
    ...
    using unique_GLFWwindow
        = std::unique_ptr<GLFWwindow, decltype([](GLFWwindow* window) { glfwDestroyWindow(window); })>;
...
public:
    static void init() noexcept(false)
    {
        ...
        unique_GLFWwindow window{
            glfwCreateWindow(s_windowWidth, s_windowHeight, s_windowName.c_str(), nullptr, nullptr)
        };
        if (!window) {
        ...
        , m_vulkan{ std::move(vulkan) }
    {
        ...
        m_fpsCounter = FpsCounter{
            ...
            [&window = m_window](double time) {
                glfwSetWindowTitle(window.get(), std::format("{} - FPS: {:.2f}", s_windowName, 1.0 / time).c_str());
            },
        };
    }

    void draw() { m_vulkan.drawFrame(); }
